...
<?php
  ...
  if (is_not_logged() == false):
    ...
    if ($uuid) {
    $req = "SELECT * FROM profile WHERE uuid = '$uuid'";
    ...
    if (($res = $data_snap->fetch(\PDO::FETCH_ASSOC)) != false) {
      ...
      $del_wp_req = "UPDATE profile SET wallpaper=null where username = '$author';";
                  ...
                  <div style="gap: 1em; justify-content: center; align-items: center; display: flex; flex-direction: column;">
                    ...
                    for ($i = 0; $i < 8; $i++):
                      ...
                      if (isset($_POST['chosen'])):
                        ...
                        $chosen_req = "UPDATE profile SET wallpaper='$wallpaper' WHERE username = '$author';";
    ...
    <?php endif;
      ...
      if (isset($pictures) && count($pictures)):
        for( $i = 0; $i < count($pictures); $i++ ):
          ...
          <?php
          if (is_not_logged() == false):
            if (isset($_POST[$delname])): // if a specific picture is deleted by current user
              $pdo = connect();
              $del_comments = "DELETE FROM comments USING picture WHERE picture_id = $id_picture;"; // delete comments' request on that specific picture
              $pdo->prepare($del_comments)->execute();
              $del_req = "DELETE FROM picture WHERE id = $id_picture;"; // delete the picture after having deleted its comments
            ...
            if (is_not_logged() == false):
              ...
              if (isset($pictures[$i]['user_id']) && $pictures[$i]['user_id'] == $user_id): // checks whether the picture has been created by current user and add trash button if so ?>
                <form action="" method="post" style="width: 100%; display: flex; align-items: baseline; justify-content: space-between;">
                  <div style="color: #7393B3; font-weight: normal; font-size: 12px; padding-top: 15px;">Created by you on <?php echo $created ?></div>
              ...
              <?php else: ?>
                <div style="color: #7393B3; font-weight: normal; font-size: 14px; padding-top: 15px;">Created by <?php echo $name ?> on <?php echo $created ?></div>
              <?php endif;
            endif; ?>
              ...
              <div style="display: flex; flex-direction: column; gap: 0.5em;">
                ...
                <?php $nb = $i+1; echo "<div style='width: 100%; align-self: center; padding: 5px; color: #7393B3; font-weight: normal; font-size: 14px;'>$nb" ?>
              </div>
              <?php if (is_not_logged() == false): ?>
